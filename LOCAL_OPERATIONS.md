...
备份分**两层**，解决的是两个完全不同的问题：

| 层级 | 位置 | 保留 | 解决什么问题 |
|---|---|---|---|
| **每日备份** | `db-backup\` | 最近 **30 份**，旧的自动删 | 「硬盘挂了 / 昨天误删了数据」 |
| **月度归档** | `db-backup\archive\` | **永久保留，从不删除** | 「税务局要查 FY2026 的账」 |

- **每天中午 12:30 自动备份**（Windows 计划任务 `ShipmentTracker-DailyBackup`）
- **每月第一次备份会自动复制一份进 `archive\`**，之后当月再跑也不会覆盖它
- 存放位置：`Brandon Coding Project\db-backup\`
  —— 这个目录在 **OneDrive 同步范围内**，所以备份会自动上云。**这是有意的**：本地硬盘挂了还能从 OneDrive 恢复。

> **为什么要多一层归档？** 马来西亚 SST 要求单据记录**保存 7 年**。只有 30 天滚动备份的话，上个月的账过一个月就永久消失了——防灾是够了，但**应付不了税务稽查**。月度归档一份约 220 KB，7 年满打满算约 **18 MB**，成本可以忽略。
>
> 归档**不会自动清理**，这是刻意的：自动删掉一份法定记录，比多占几十兆硬盘严重得多。哪天真要清理，人工判断后手动删。
...
> ⚠️ **注意：这一步只证明「备份文件存在」，不证明「备份能恢复」。** 两者是两回事——最常见的备份事故就是天天备份成功，真要恢复时才发现文件是坏的。

### 恢复演练（每季度做一次）

**不碰正式库**的做法：把备份恢复进一个临时库，核对数据，再删掉。

```bash
node scripts/verify-backup.js
```

它会自动挑最新的备份，建临时库、恢复、统计各表行数、然后**无论成功失败都把临时库删掉**。全程只读正式库的连接信息，不会写入。

**上次演练：2026-08-08，通过**（0 错误，客户/报价/发票/用户行数全部对上）。
